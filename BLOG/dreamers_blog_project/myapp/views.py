...
def admin_add_category(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('edit-category')  # Redirect after successful submission
    else:
        form = CategoryForm()  # Create a new form instance for GET requests

    return render(request, 'add_category.html', {'form': form})  # Render your HTML template



def edit_category(request, id):
    category = get_object_or_404(Category, id=id)  # Fetch the category to edit
    if request.method == 'POST':
        form = CategoryForm(request.POST, request.FILES, instance=category)  # Bind the form to the existing instance
        if form.is_valid():
            form.save()  # Save changes to the category
            return redirect('edit-category', id=category.id)  # Redirect to the edit page or a different page
    else:
        form = CategoryForm(instance=category)  # Load the form with existing category data
